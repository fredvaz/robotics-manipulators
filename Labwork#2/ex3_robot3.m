...
q = [pi/4  2 0 0 pi/4 pi/4];


disp('############################################################################################')
% Robot 3 [RPR + RRR] - Matriz dos parametros de Denavith-Hartenberg: PJ_DH3
disp(' ')
disp('Robot 3 - Matriz dos parametros de Denavith-Hartenberg: PJ_DH')
disp(' ')

%           thetai  di   ai  alphai   offseti   sigmai
PJ_DH = [   theta1   0    0    pi/2        0        1;  % Junta de rotação
                 0  d2    0   -pi/2        2        0;  % Junta prismática
            theta3   0    0       0        0        1;  % Junta de rotação
            theta4   0    0   -pi/2        0        1;  % Punho esferico
            theta5   0    0    pi/2        0        1;  % Punho esferico
            theta6  L1    0       0        0        1]; % Punho esferico
...
while(select ~= sair)
    ...
    if select == 1
        ...
        %junta rotacional 3
        L(3) = Link('d',eval(PJ_DH(3,2)), 'a', eval(PJ_DH(3,3)), 'alpha', eval(PJ_DH(3,4)));
        ...
        robot = SerialLink(L, 'name', 'Robot3');
        
        figure('units','normalized','outerposition',[0 0 1 1]);
        robot.teach(qh, 'workspace', [-2 2 -2 2 -1 2], 'reach',1); % "drive" a graphical robot using a graphical slider panel
    ...
    if select == 2
        ...
        T0_H_values = eval(subs(T0_H, [d2 theta1 theta3 theta4 theta5 theta6], q));
   ...
   %% c) Modelo inverso dos Robots
    if select == 3
        ...
        PCI = inverse_kinematics_R3(T0_H);
        
        %substitui na matriz simbólica
        T0_H_values_inv = eval(subs(T0_H, [d2 theta1 theta3 theta4 theta5 theta6], PCI));
        ...
        T0_H_Toolbox_inv = eval(subs(T_0G, [d2 theta1 theta3 theta4 theta5 theta6], PCI_Toolbox));
